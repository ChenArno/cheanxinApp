<template>
  <div class="vux-rater">
    ...
    <a class="vux-rater-box" v-for="i in max" @click="handleClick(i-1)" :class="{'is-active':currentValue > i-1}"
      ...
      <span class="vux-rater-inner iconfont icon-star"><span class="vux-rater-outer"
                                                             :style="{color: activeColor, width: cutPercent + '%'}"
                                                             v-if="cutPercent > 0 && cutIndex === i-1"></span></span>
    </a>
  </div>
</template>
...
<script>
  export default {
    name: 'rater',
    created () {
      this.currentValue = this.value * 5 / this.total
    },
    mounted () {
      this.updateStyle()
    },
    ...
    watch: {
      currentValue (val) {
        this.updateStyle()
        val = val * this.total / 5;
        this.$emit('input', val)
      },
      value (val) {
        this.currentValue = val * 5 / this.total
      }
    }
  }
</script>
...
<style>
  ...
  .vux-rater-box.is-active .icon-star::before {
    content: '\e65f';
  }
</style>
